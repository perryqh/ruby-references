...
use super::{
    ...
    ParsedDefinition, PreprocessedFile, Range, UnresolvedReference,
};
...
pub(crate) fn process_from_path(path: &Path) -> anyhow::Result<PreprocessedFile> {
    let contents = file_read_contents(path)?;
    Ok(process_from_contents(contents, path))
}

pub(crate) fn process_from_contents(contents: String, path: &Path) -> PreprocessedFile {
    ...
    for d in &collector.definitions {
        ...
        for (index, _) in parts.iter().enumerate() {
            ...
            if !definition_to_location_map.contains_key(combined) {
                definition_to_location_map.insert(combined.to_owned(), d.location);
            }
        }
    }
