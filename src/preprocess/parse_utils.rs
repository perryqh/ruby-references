...
use super::{inflector_shim, ParsedDefinition, Range, UnresolvedReference};
...
pub fn fetch_const_const_name(node: &nodes::Const) -> Result<String, ParseError> {
    match &node.scope {
        Some(s) => {
    ...
    if is_association {
        ...
        if let Some(Node::Sym(d)) = first_arg {
            if name.is_none() {
                ...
                name = Some(inflector_shim::to_class_case(
